...
def index():
    ...
    for ex in exerciseDB:
        ...
        if ex.learntype not in user_exercise[db_topic.chapter_id][db_topic.number]:
            user_exercise[db_topic.chapter_id][db_topic.number][ex.learntype] = int(
                float(ex.percent))
        else:
    ...
    if request.method == 'POST':
        ...
        for select in range(1, int(quiz_number)+1):
            select_choice.append(
                request.form['choice'+str(select)].replace(".", ""))
        ...
        # commit to database
        db_topicid = Topic.query.filter_by(
            chapter_id=chapterid, number=topicid).first().id
        userid = User.query.filter_by(email=current_user.email).first().id

        # Add to Exercise table
        exerciseDB = Exercise.query.filter_by(
            user_id=userid, topic_id=db_topicid).first()
...
def chapter_summary():
    ...
    for ch in allchapter:
        ...
        for tp in chapter_topic:
            ...
            user_result = Exercise.query.filter_by(
                user_id=userid, topic_id=tp.id).all()
            ...
            # if done posttest
                ...
                for vs in vark_earn_score:
                    chapter_sum[ch.id][vs] = int(
                        (vark_earn_score[vs] / vark_full_score[vs]) * 100)
        ...
        for topic in chapter_topic:
            ...
            if topic.number != 'P' and topic.number != 'T':
                ...
                for media in ['V', 'A', 'R', 'K', 'Ex Count', 'All score']:
                    sheet.cell(row=content_row,
                               column=content_start_column).value = media
                    content_start_column += 1
            else:
                chapter_merge += 2
                topic_merge = 2
                sheet.cell(row=content_row,
                           column=topic_start_column).value = 'Score'
                sheet.cell(row=content_row,
                           column=topic_start_column+1).value = 'Time'

            topic_end_column = topic_start_column + topic_merge - 1
            sheet.merge_cells(start_row=topic_row, start_column=topic_start_column,
                              end_row=topic_row, end_column=topic_end_column)
            sheet.cell(row=topic_row,
                       column=topic_start_column).value = topic.number
            topic_start_column = topic_end_column + 1

        #print("chapter merge : ", chapter_merge)
        chapter_row = 1
        end_column = start_column + chapter_merge - 1
        sheet.merge_cells(start_row=chapter_row, start_column=start_column,
                          end_row=chapter_row, end_column=end_column)
    ...
    for user in alluser:
        ...
        if user.user_type != 'Admin':
            ...
            for uinfo in [firstname, lastname, gender, age, email]:
                sheet.cell(row=user_row_start,
                           column=user_column).value = uinfo
                user_column += 1

            for chapter in allchapter:
                chapter_topic = Topic.query.filter_by(
                    chapter_id=chapter.id).all()
                for topic in chapter_topic:
                    user_excercise = Exercise.query.filter_by(
                        user_id=user.id, topic_id=topic.id).first()
                    if user_excercise:
                        if topic.number == 'P' or topic.number == 'T':
                            sheet.cell(
                                row=user_row_start, column=user_column).value = user_excercise.percent
                            user_column += 1
                            # type = User, not have time stramp in exercise
                            if user.user_type == 'User':
                                sheet.cell(row=user_row_start,
                                           column=user_column).value = '-'
                                user_column += 1
                            else:
                                sheet.cell(
                                    row=user_row_start, column=user_column).value = user_excercise.time
                                user_column += 1
                        else:
                            for media in ['V', 'A', 'R', 'K']:
                                media_point = Exercise.query.filter_by(
                                    user_id=user.id, topic_id=topic.id, learntype=media).all()
                                    ...
                                    for mp in media_point:
                                        if highest_percent < int(float(mp.percent)):
                                            highest_percent = int(
                                                float(mp.percent))
                                    sheet.cell(
                                        row=user_row_start, column=user_column).value = highest_percent
                                    user_column += 1
                                else:
                                    sheet.cell(row=user_row_start,
                                               column=user_column).value = '-'
                            ...
                            topic_score = Exercise.query.filter_by(
                            ...
                            for ts in topic_score:
                                ...
                                if user.user_type == 'User':
                                    score_vark += ts.learntype + ':' + \
                                        str(int(float(ts.percent)))
                                    num += 1
                                # User1 --> learntype : score : time, ..
                                else:
                                    score_vark += ts.learntype + ':' + \
                                        str(int(float(ts.percent))) + \
                                        ':' + str(int(float(ts.time)))
                                    num += 1

                                if num < len(topic_score):
                                    score_vark += ', '
                            sheet.cell(row=user_row_start, column=user_column).value = len(
                                topic_score)
                            user_column += 1
                            sheet.cell(row=user_row_start,
                                       column=user_column).value = score_vark
                            user_column += 1
            user_row_start += 1
